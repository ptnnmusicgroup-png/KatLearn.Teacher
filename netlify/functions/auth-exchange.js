...
const allowedOrigins = new Set(['https://teacher-katlearn.netlify.app','https://lms-katlearn.netlify.app']);
...
export default async request=>{
 ...
 try{
  ...
  if(target==='teacher'&&!isAdmin&&role!=='teacher')return Response.json({role:'student',error:'STUDENT_ACCOUNT'},{status:403,headers:headers(origin)});
  ...
  return Response.json({role:role==='teacher'||isAdmin?'teacher':'student',customToken},{status:200,headers:headers(origin)});
 }catch(error){console.error('[KatLearn SSO]',error);return Response.json({error:'Invalid or expired session'},{status:401,headers:headers(origin)})}
};
