(function(){
  ...
  async function verifyTeacher(uid){try{const teacherSnap=await api.getDoc(api.doc(db,'users',uid));if(!teacherSnap.exists())throw new Error('Không tìm thấy hồ sơ giáo viên.');const profile=teacherSnap.data()||{};await api.updateDoc(api.doc(db,'users',uid),{role:'teacher','teacherVerification.status':'verified','teacherVerification.verifiedAt':Date.now(),'teacherVerification.verifiedBy':user.uid,updatedAt:Date.now()});const schoolId=String(profile.schoolId||'').trim();if(schoolId)await api.updateDoc(api.doc(db,'schools',schoolId),{catalogStatus:'active',updatedAt:Date.now()}).catch(()=>{});const classIds=Array.isArray(profile.classIds)?profile.classIds.slice(0,20):[];for(const classId of classIds)await api.updateDoc(api.doc(db,'schools',schoolId,'classes',classId),{catalogStatus:'active',updatedAt:Date.now()}).catch(()=>{});toast('✓ Đã cấp quyền giáo viên');renderPending()}catch(e){toast('Không thể xác minh: '+e.message)}}
