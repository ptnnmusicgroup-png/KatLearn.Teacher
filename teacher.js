...
async function getMembers(cls){const snap=await fb.getDocs(fb.collection(db,'classes',cls.id,'members'));return snap.docs.map(d=>({id:d.id,uid:d.id,...d.data(),uid:String(d.data()?.uid||d.id)}))}
...
async function loadProgress(){if(!isTeacher())return;if(!selectedClass&&classes[0])selectedClass=classes[0];const stat=selectedClass?await classStats(selectedClass):null;if(!stat||!stat.rows.length){$('#progressTable').innerHTML='<div class="empty">Chọn một lớp có học sinh để xem tiến độ.</div>';return}$('#progressTable').innerHTML=`<p class="progress-note">${esc(selectedClass.name)} · xếp hạng theo số câu đúng, sau đó đến độ chính xác và XP.</p>`+stat.rows.map((s,i)=>`<div class="student-row"><b>#${i+1}</b><span class="student-avatar">${esc((s.displayName||'K')[0]).toUpperCase()}</span><div><strong>${esc(s.displayName||'KatLearn Student')}</strong><small>${esc(s.email||'')} · ${accuracy(s)}% chính xác</small></div><div class="student-score"><b>${Number(s.correctAnswers||0)} câu đúng</b><small>⚡ ${Number(s.energy||0).toLocaleString()} XP · 📚 ${Number(s.totalWords||0)} từ</small></div></div>`).join('')}
